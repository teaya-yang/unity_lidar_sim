...
{
    // True → TrafficManager will Destroy this NPC on the next despawn pass.
    // Set in concrete class when the vehicle leaves the map or completes a limited route.
    bool ShouldDespawn { get; }

    // World-space AABB used by NpcSpawner.IsSpawnable() to reject overlapping spawns.
    Bounds Bounds { get; }

    // Called by TrafficManager after Instantiate().
    ...
    void OnNpcInitialize(TaxiwayLane startLane, Transform[] egoVehicles);
}
